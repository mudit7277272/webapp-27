...
import { MousePointer2, Loader2, InfoIcon, PowerIcon } from "lucide-react";
...
const ParticleAnimation = () => {
  ...
  const [animationContainerDimensions, setAnimationContainerDimensions] =
    useState({ width: window.innerWidth, height: window.innerHeight }); // State for container dimensions
  ...
  const handleToggleRepulsion = useCallback(() => {
    const now = Date.now();
    if (now - lastToggleTimestampRef.current > 300) {
      // Debounce time of 300ms
      ...
      setRepulsionEnabled((prev) => {
        ...
        if (nextRepulsionState) {
          ...
          toast.info("Repulsion Effect Disabled", {
            description:
              "Particles will gently return to their original positions.",
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const radius =
        Math.sqrt(Math.random()) * Math.min(usableWidth, usableHeight) * 0.5; // Random radius for particle placement
      ...
      const particleRadius =
        MIN_PARTICLE_RADIUS +
        Math.random() * (MAX_PARTICLE_RADIUS - MIN_PARTICLE_RADIUS); // Random radius for each particle
      const geometry = new THREE.SphereGeometry(particleRadius, 32, 32); // Sphere geometry for particles
      const color =
        PARTICLE_COLORS[Math.floor(Math.random() * PARTICLE_COLORS.length)]; // Random color from palette
      const material = new THREE.MeshPhysicalMaterial({
        // Physical material for realistic lighting
        color: color,
    ...
    const handleMouseMove = (event: { clientX: number; clientY: number }) => {
      ...
      mousePositionRef.current.x =
        ((event.clientX - rect.left) / window.innerWidth) * 2 - 1; // Normalize mouse X position
      mousePositionRef.current.y =
        -((event.clientY - rect.top) / window.innerHeight) * 2 + 1; // Normalize mouse Y position
    };
      ...
      particles.forEach((particle) => {
        ...
        if (perspectiveCameraRef.current) {
          const mouseVector = new THREE.Vector3(
            ...
            mousePositionRef.current.y,
            0.5
          ); // Create vector from mouse position
          ...
          const mouseWorldPosition = cameraPosition
            .clone()
            .add(direction.multiplyScalar(distance)); // Calculate mouse position in world coordinates
          ...
          if (dist < MOUSE_REPULSION_RADIUS && isRepulsionEnabledRef.current) {
            ...
            particle.userData.vx -=
              Math.cos(angle) * force * (1 + particle.position.z * 0.5); // Apply force in opposite direction of mouse
            particle.userData.vy -=
              Math.sin(angle) * force * (1 + particle.position.z * 0.5); // Apply force in opposite direction of mouse
          ...
          particle.userData.vx +=
            (originalX - particle.position.x) * ATTRACTION_FORCE;
          particle.userData.vy +=
            (originalY - particle.position.y) * ATTRACTION_FORCE;
          particle.userData.vz +=
            (originalZ - particle.position.z) * ATTRACTION_FORCE * 0.5;

          // Gravity force (subtle downward pull)
          particle.userData.vy +=
            GRAVITATIONAL_FORCE * (1 - particle.position.z * 0.5);
          ...
          const velocity = Math.sqrt(
            particle.userData.vx ** 2 + particle.userData.vy ** 2
          );
          if (velocity > 0.001) {
            const dragForce = MOVEMENT_DRAG * velocity;
            particle.userData.vx -=
              (particle.userData.vx / velocity) * dragForce;
            particle.userData.vy -=
              (particle.userData.vy / velocity) * dragForce;
          }
          particle.userData.vz *= 1 - MOVEMENT_DRAG;

          // Clamp velocities to maximum velocity
          particle.userData.vx = clamp(
            particle.userData.vx,
            -MAX_PARTICLE_VELOCITY,
            MAX_PARTICLE_VELOCITY
          );
          particle.userData.vy = clamp(
            particle.userData.vy,
            -MAX_PARTICLE_VELOCITY,
            MAX_PARTICLE_VELOCITY
          );
          particle.userData.vz = clamp(
            particle.userData.vz,
          ...
          particle.position.z = clamp(
            particle.position.z + particle.userData.vz,
            -0.5,
            0.5
          ); // Clamp Z position for depth boundaries

          // Keep particles within container bounds
          particle.position.x = clamp(
            particle.position.x,
            -animationContainerDimensions.width / 2,
            animationContainerDimensions.width / 2
          );
          particle.position.y = clamp(
            particle.position.y,
            -animationContainerDimensions.height / 2,
            animationContainerDimensions.height / 2
          );
  ...
  return (
    ...
    >
      ...
      {/* Toaster for notifications, aria-live for screen readers */}
          ...
          <div className="w-full sm:w-auto mb-4 sm:mb-0 text-center">
            ...
            <p className="mt-2 flex items-center gap-2 font-medium text-sm sm:text-base whitespace-nowrap  bg-white/80 backdrop-blur-sm py-1 px-3 rounded-md shadow-sm">
              <InfoIcon
                ...
                aria-hidden="true"
              />{" "}
              {/* Icon, aria-hidden for screen readers */}
      ...
      {isLoading && (
        <div
          className="absolute inset-0 flex items-center justify-center z-50 bg-gray-900/75 backdrop-blur-sm"
          aria-live="polite"
        >
          {" "}
          ...
          <div className="flex flex-col items-center gap-4">
            <Loader2
              ...
              aria-hidden="true"
            />{" "}
            {/* Loader icon, aria-hidden for screen readers */}
            <div className="text-white text-lg font-medium">
              Loading Particle Cluster...
            </div>
          </div>
        </div>
      )}
      ...
      <div className="fixed bottom-6 right-6 z-10 group">
        <Button
          ...
          aria-label={`Toggle Repulsion Effect. Currently ${
            isRepulsionEnabled ? "Enabled" : "Disabled"
          }`}
        >
          <PowerIcon
            className={`w-4 h-4 ${
              isRepulsionEnabled ? "text-blue-600" : "text-gray-600"
            }`}
            aria-hidden="true"
          />
